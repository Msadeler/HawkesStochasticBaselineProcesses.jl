...
function rand(hsb::HawkesStochasticBaseline, condition::Union{Float64,Int64})
    dimCov = length(hsb.X₀)>1 ? MultiDimCov : UniDimCov
    rand(hsb, condition, dimCov)
...
function rand(hsb::HawkesStochasticBaseline, max_time::Float64, ::Type{MultiDimCov})
    ...
    while t<max_time
        ...
        ## update covariable
        Xₜ = Xₖ₋₁ .+ hsb.diffusion(Xₖ₋₁,t).*(t-t⁻) .+ hsb.drift(Xₖ₋₁,t).*rand(Normal(0, sqrt(t-t⁻)), length(hsb.X₀) )
